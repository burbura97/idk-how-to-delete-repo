...
const Calendar = () => {
  ...
  const loadEvents = async () => {
    ...
    try {
      ...
      // Even if there's an error, try to get mock data
      const mockBookings = calComService.getMockBookings();
      const processedMockEvents = mockBookings.map(booking => ({
        id: booking.id,
        title: booking.title,
        start: new Date(booking.startTime),
        end: new Date(booking.endTime),
      ...
      setError('Using mock data - Cal.com API unavailable');
    } finally {
      setLoading(false);
    }
  };
  ...
  const getEventsForDate = (date) => {
    ...
    return events.filter(event => {
      // Cal.com format - event.start is a Date object
      if (event.start instanceof Date) {
        const eventDate = event.start.toISOString().split('T')[0];
        return eventDate === dateStr;
      }
      // Google Calendar format (legacy)
      if (event.start.date) {
